...
PRES_BIB <- file.path(script_dir, "Bibliography", "cv_presentations.bib")
...
TYPE_MAP <- c(journalArticle = "article", bookSection = "incollection",
              report = "techreport", book = "book", presentation = "misc")
...
FIELD_MAP <- c(
  ...
  meetingName = "howpublished"  # conference/venue name for presentations
)
...
# Write a list of records to a .bib file. order_keys controls field order (and
# which fields are emitted); entries are keyed <key_prefix><n>.
write_bib <- function(recs, out_path, order_keys, key_prefix, label) {
  lines <- c(
  ...
  for (i in seq_along(recs)) {
    rec <- recs[[i]]
    lines <- c(lines, sprintf("@%s{%s%d,", rec$`_type`, key_prefix, i))
    body <- character()
    for (k in order_keys) {
      v <- rec[[k]]
  ...
  cat(sprintf("Wrote %d %s to %s\n", length(recs), label, out_path))
}
...
load_items <- function(con) {
  items <- dbGetQuery(con, "
    ...
    WHERE di.itemID IS NULL
      AND it.typeName IN ('journalArticle','bookSection','report','book','presentation')
  ") |> as_tibble()
  ...
  creators <- dbGetQuery(con, sprintf("
    ...
    WHERE ic.itemID IN (%s) AND ct.creatorType IN ('author', 'presenter')
...
main <- function() {
  ...
  pub_keys <- c("author", "title", "year", "journal", "booktitle", "institution",
                "publisher", "volume", "number", "pages", "address", "doi", "keywords")
  write_bib(recs, OUT_BIB, pub_keys, "cv", "publications")

  # Presentations: talks Sippel gave (Zotero type "presentation", presenter =
  # Sippel). Written to their own bib so they never mix with the publication
  # list. Not de-duplicated — the same talk given at different meetings is a
  # distinct entry. Newest first, then by title.
  pres <- items |>
    ...
    pmap(function(itemID, type, modified, fields, authors, tags) {
      ...
      authors_flat <- str_c(item$authors$last, ", ", item$authors$first, collapse = " and ")
      if (is.na(authors_flat)) authors_flat <- ""
      if (!str_detect(str_to_lower(authors_flat), fixed(str_to_lower(AUTHOR_SURNAME))))
        return(NULL)
      rec <- to_record(item)
      if (is.null(rec) || !nzchar(rec$title)) return(NULL)
      rec
    ...
    compact()
  pres <- pres[order(-as.integer(map_chr(pres, "year")), map_chr(pres, "title"),
                     method = "radix")]

  pres_keys <- c("author", "title", "year", "howpublished", "address")
  write_bib(pres, PRES_BIB, pres_keys, "pres", "presentations")
}
